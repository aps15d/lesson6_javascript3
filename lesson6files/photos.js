...
function previewFive() {
   ...
   //clone fgure element for fifth image and edit to be first image
   var firstFigure = lastFigure.cloneNode(true);
   
   firstFigure.id = "fig1";
   firstFigure.style.right = " ";
   firstFigure.style.left = "45.px";
   ...
   //disable the 'Show more images' n\button after it has been selected once
...
function createEventListeners() {
    ...
    //add an event handler to the specififed document element (leftarrow)
    //the leftArrow() function will be execuated when the user clicks the left navigation arrow button
    leftarrow.addEventListener("click", leftArrow, false);

    //declare variable for document element with id of rightarrow, which is in the html document
    var rightarrow = document.getElementById("rightarrow");

    //add an event handler to the specififed document element (rightarrow)
    //the rightArrow() function will be execuated when the user clicks the right navigation arrow button
    rightarrow.addEventListener("click", rightArrow, false);

    //declare a variable for the element fiveButton
    var showAllButton = document.querySelector("#fiveButton p");
   
    //add event listen to show showAllButton element
